...
//! factories (generated by the `native_api_factory!` / `subsonic_api_factory!`
//! macros in app_service.rs) so the auth dance stays consistent.
...
impl<'a> LibraryOrchestrator<'a> {
    ...
    /// `SongsApiService` on demand — mirrors the `AppService::songs_api()`
    /// factory's auth dance (generated by `native_api_factory!` in app_service.rs).
