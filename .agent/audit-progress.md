...
| 12 | Type-level queue invariants (IG-1 + IG-2 + IG-4 + IG-5) | 🟡 partial | Lane A landed `9a8fa7c` (IG-1: `get_queue_mut` deleted; IG-2: `insert_song_at` renamed to `insert_song_and_make_current` + body refactored to delegate through `insert_songs_at`). Still open: IG-4 (no `PeekedQueue` borrow-guard), IG-5 (no `QueueWriteGuard`). Lanes B and C cover the rest. |
