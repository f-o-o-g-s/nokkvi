...
| 12 | Type-level queue invariants (IG-1 + IG-2 + IG-4 + IG-5) | 🟡 partial | Lane C (IG-5 + queue-side IG-3) landed `1d2d9c8..4e2c960` (2026-05-08): `QueueWriteGuard` in `data/src/services/queue/write_guard.rs`; every mutator in `mod.rs` runs through `let mut tx = self.write(); …; tx.commit_save_*()`; `set_repeat`/`toggle_consume` now clear `queued` (locked by `set_repeat_clears_queued`/`toggle_consume_clears_queued`). Lanes A (IG-1 + IG-2 — `get_queue_mut` deletion + `insert_song_at` rename) and B (IG-4 — `PeekedQueue`) still open. |
...
| IG-4 | `peek_next_song` → `transition_to_queued` discipline is doc-only | ❌ open | No `PeekedQueue` borrow-guard. |
| IG-5 | `clear_queued()` after every queue mutation enforced by 11+ explicit call sites | ✅ done | Lane C (2026-05-08), commits `1d2d9c8..4e2c960`. `QueueWriteGuard` in `data/src/services/queue/write_guard.rs`: every mutator in `mod.rs` runs through `let mut tx = self.write(); …; tx.commit_save_*()`. `Drop` is the safety net for `?` / panic paths. `clear_queued` call sites in `mod.rs`: 11 → 0. |
