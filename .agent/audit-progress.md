...
| IG-3 | Mode-toggle methods (`toggle_shuffle`, `set_repeat`, `toggle_consume`) don't compel `engine.reset_next_track()` | ✅ done | Queue-side half closed in queue-typestate-igs Lane C `4e2c960` (2026-05-08): all three mode toggles now go through `QueueWriteGuard` and clear `queued` consistently. Engine-side closed in audio-engine-typestate-igs Lane C `3c58fa9..7a9c1c8` (2026-05-08): `types/mode_toggle.rs::ModeToggleEffect` is a `#[must_use]` token returned by `QueueManager::toggle_shuffle`/`set_repeat`/`toggle_consume`; its only consumer is `effect.apply_to(&engine).await`, so a future caller cannot toggle a queue mode and silently skip the gapless-prep reset. `PlaybackController` site count for `engine.reset_next_track()`: 3 → 0. |
...
| IG-11 | RG-stash + `set_source` / `load_track` + `play()` sequencing — 4 hand-paired sites | ✅ done | audio-engine-typestate-igs Lane C `576088b..83bdc4b` (2026-05-08): added `CustomAudioEngine::load_track_with_rg(url, rg)` (engine.rs:1084) that pairs the renderer RG-stash and the source-update atomically; migrated all seven hand-paired sites (5 in `playback_controller.rs`, 2 in `services/playback.rs`). `set_pending_replay_gain` site count in `playback_controller.rs` + `services/playback.rs`: 7 → 0. The crossfade-side `set_pending_crossfade_replay_gain` stays public — it's the next-track slot, distinct from primary-stream RG-stash. |
...
| 13 | Crossfade armed/active dual-flag in `AudioRenderer` | ✅ done | Lane B `b1603c4` (2026-05-08): folded into `CrossfadeState` enum (see IG-8). The `crossfade_active` / `crossfade_armed` bools and the 3 `crossfade_armed_*` fields are gone — `is_crossfade_active()` / `is_crossfade_armed()` now `matches!` the variant. |
