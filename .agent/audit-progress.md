...
2. **§7 #8 — Loader-result `LoaderTarget` trait** (M effort, collapses the 5 parallel `handle_*_loaded` bodies). Now more accessible: `SongSource::resolve` from §7 #7 demonstrates the entity-dispatch pattern across 4 fetch paths in 1 enum + 1 dispatcher; `LoaderTarget` reuses the same shape on the result side.
3. **§7 #3 — `define_view_columns!` persist emission** (M effort, the most-frequent feature edit; persist-arm omission fails silently on relaunch).
