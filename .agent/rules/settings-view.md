...
- **Typed config write routing** (`src/config_writer.rs`):
  ...
  - Routes through `route_write` (helper at the top of `update/settings.rs`) which forks scalar vs sentinel writes; sentinel paths use `SentinelKind` (in `views/settings/sentinel.rs`) instead of stringly keys
