...
- **Pre-play radio→queue transition**: `guard_play_action()` at the top of every play handler transitions radio playback back to queue mode (returns `None` to let the play proceed). It returns `Option<Task>` so a future block could short-circuit a play, but the former playlist-edit block was removed — the playlist editor owns a track buffer decoupled from the live queue, so a play can't disturb an edit.
