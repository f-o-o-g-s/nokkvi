...
| Base Slot List | `base_slot_list_layout.rs` | Shared layout scaffolding (`base_slot_list_layout{,_with_handle}`, `BaseSlotListLayoutConfig`). `base_slot_list_empty_state()` lives in `widgets/mod.rs` and routes through it to preserve the root widget type (and `text_input` focus) across results/no-results |
