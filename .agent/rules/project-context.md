...
External repos cloned locally for read-only reference (not part of this project). `reference-{name}/`: `iced`, `iced-apps`, `iced-book`, `iced-docs`, `symphonia`, `feishin`, `lookas`, `lucide` (icons), `mimalloc`, `mimalloc-rust`, `mpris`, `mpv`, `navidrome`, `rmpc`, `rmpc-docs`, `rodio`.
