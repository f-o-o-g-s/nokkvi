...
`SongPool` (HashMap) + `Queue` ordering (`rows: Vec<QueueRow{song_id, entry_id}>` + `order: PlayOrder` + `current_order`). Modules: `services/queue/{mod, navigation, order, write_guard}.rs`. The typed model (2026-07 queue domain redesign):
...
- **The physical playhead is DERIVED**: `Queue::current_index()` returns `order[current_order]`; there is no stored index field, so `order[current_order] != current_index` is unrepresentable. Move the playhead via `set_cursor_to_row(row)` / `rebuild_order_and_set_cursor(row)` / direct `current_order` writes to an order slot — never by tracking a physical index in parallel.
