...
`LivePlayerSettings` (renamed from `PlayerSettings` to avoid colliding with the persisted `PersistedPlayerSettings`) is the live in-memory union of every user-tunable knob; split into per-domain submodules under `data/src/types/player_settings/` (`appearance`, `artwork`, `library`, `navigation`, `playback`, `slot_list`, `strip`, `verbose`, `visualizer`). `TomlSettings` is the on-disk shape of `[settings]`. Notable knobs include `font_family`, `library_page_size`, `artwork_resolution`, `volume_normalization` + ReplayGain, per-view column flags (`{view}_show_*`), artwork column, tray, nav, `slot_row_height`, `track_info_display`, default-playlist, strip. Read the structs for the full set.
