...
Adjacent enum macros: `define_labeled_enum!` (`data/src/types/labeled_enum.rs`) for label/wire-string-backed enums (used by `QueueSortMode`, the visualizer enums, etc.); `atomic_u8_enum!` (`src/atomic_u8_enum.rs`) for the nine `UiModeFlags` atomics. Both flow through the same `from_label` / `as_label` shape so a TOML typo can't silently fall through anymore.
