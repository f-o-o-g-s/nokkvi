...
- [ ] **CLI / IPC**: an externally-triggerable action gets a verb in the `define_commands!` macro (`src/update/ipc.rs` — emits `IPC_KNOWN_COMMANDS` + `CLI_ARGS`) plus a row in the enforced verb-list test (`src/update/tests/ipc.rs`); the wire layer lives in `nokkvi-ipc/` and must stay iced-free
