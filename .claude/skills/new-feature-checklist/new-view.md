...
7. Render the page in `src/app_view.rs`, and size its stored slot count from the chrome it renders:
   - Add a `LibraryPage` variant and list it in `LibraryPage::ALL` and in `all_slot_list_commons_mut` (`src/update/navigation.rs`). `library_pages_and_the_queue_cover_every_pooled_page` pins the two lists together, and the exhaustive matches in `library_page_common{,_mut}`, `library_page_in_browsing_pane`, and `library_page_chrome` walk you through the page's pane, header menus, and select flag.
   - Give `{Name}ViewData` a `chrome: SlotListChrome` built from `self.library_page_chrome(LibraryPage::{Name})`, with `window_width` / `window_height` taken from its pane size. The view reads `data.chrome` for the header's `collapsed`, the select-all bar, `slot_list_chrome` (`chrome.height()`), and `with_dynamic_slots(data.window_height, data.chrome.effective())`. `resync_slot_counts` sizes the stored count from the same call, which keeps drags, find-and-expand, and the scrollbar thumb on the rendered rows.
