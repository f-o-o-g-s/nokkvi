...
- `impl_expansion_update!` macro — handles the shared search/sort/expand/hover/toolbar arms for expansion views
- `synth_set_offset_message(&self, offset: usize) -> Option<Message>` — builds a per-view `SlotList(SlotListPageMessage::SetOffset(offset, default_modifiers))` message; used by `handle_seek_settled` to trigger artwork prefetch after scrollbar seek.
- `reload_message(&self) -> Option<Message>` — emits the view's "reload from server" message (the `R` hotkey + manual refresh button bind through this). Reloadable views return `Some(...)`; views without server backing return `None`.
...
Every per-view `*Message` enum carries a `SlotList(SlotListPageMessage)` variant as the unified slot-list carrier. All slot-list state mutations route through `SlotListPageMessage`.

Harbour (`HarbourPage`, `View::Harbour`) is the whole-library home view (collapsible shelves + whole-library search) and the default start view. Similar has no `View` enum variant — it only renders inside the browsing panel.

## SlotListPageState

Shared by every slot-list view: search query, scroll position, focus index. Visible slot count is computed dynamically from window height (always odd, capped at `MAX_SLOT_COUNT = 29`). Each page's stored `slot_count` comes from the same chrome function its `view()` feeds to `with_dynamic_slots`, with each input derived once on `Nokkvi` and handed to both: library pages read `SlotListChrome` (`widgets/slot_list.rs`) from `library_page_chrome(LibraryPage)` via their view data's `chrome`; the queue reads `queue_chrome_height` / `queue_effective_chrome` (`views/queue/view.rs`) over `queue_chrome_inputs()`; the playlist editor reads `editor_chrome_height` / `editor_effective_chrome`. The inputs are the header's auto-hide collapse (an open header menu holds it expanded), the select-all bar, the queue's banner terms, and the pane the page renders in (split-view queue pane, or the browsing pane's width and tab-bar-less height). `resync_slot_counts()` runs after every root `update` message and at the top of `handle_queue`; the drag mappers, find-and-expand landing, the scrollbar thumb, and `get_center_item_index` all read the stored count. Prefetch radius = `slot_count + MIN_PREFETCH_BUFFER` (3 by default).
...
`SlotListMessage` in `app_message.rs` carries global slot-list actions dispatched by hotkeys and scrollbar timers; variant semantics are doc-commented at the declaration. Root dispatch is in `handle_slot_list_message` (`update/slot_list.rs`). Each hotkey arm fans out to a per-view `Message::Albums(AlbumsMessage::SlotList(SlotListPageMessage::NavigateUp))` (and so on for every view), so the actual state mutation is always done by the per-view update handler.

### Per-view SlotList(SlotListPageMessage) carrier

Every per-view message enum carries a unified `SlotList(SlotListPageMessage)` variant (e.g., `AlbumsMessage::SlotList(…)`). `SlotListPageMessage` (in `widgets/slot_list_page.rs`) enumerates all slot-list actions; variant semantics are doc-commented at the declaration. The non-obvious ones: `ActivateCenter(bool)` — `true` forces a one-shot Shuffle Play, `false` honors the `enter_shuffle` setting. The slot-hover variants are published by per-slot `mouse_area::on_enter` / `on_exit` and land on `SlotListView::hovered_slot` (via `SlotHoverCallback` in `widgets/slot_list.rs`) so cross-pane drag resolves "cursor over which slot" structurally rather than from chrome math; `HoverExitSlot` is idempotent (only clears when its payload still matches). The three `Toolbar*` variants drive the auto-hide toolbar's reveal-lock state (see SlotListPageState above).
...
### Horizontal arrows (seek vs sort)

Bare Left/Right are Seek Backward / Seek Forward; Shift+Left/Right cycle the sort mode. Both actions call `Nokkvi::horizontal_arrow_context` (`update/hotkeys/navigation.rs`) as their FIRST statement, so wherever the arrows already EDIT something the edit wins whichever pair was pressed: the Trawl tray while its modal is open, `EditLeft`/`EditRight` in Settings. Everywhere else the sort cycle plays its SFX and reveals the toolbar, while a seek does neither — a held key repeats ~25x/s, so both would be stranded. The pure `horizontal_arrow_owner()` names the owning context and is what tests assert. The smart-playlist rules editor and its date picker take LITERAL arrows for `CycleCell` before any binding lookup, so bare arrows never seek there (a rebound seek key does).

## Inline Expansion

Generic `ExpansionState<C>` + `SlotListEntry<P, C>`. Center-entry resolution is centralized in `views/expansion.rs`. (gotchas: see gotchas.md "Misc" — expansion sort state.)
...
- **Tracks** — the ordered-track editor. Edit-bar header: eyebrow + name input + comment input (left), public/lock toggle + remove-duplicates + save + discard (right). Save via `handle_save_playlist_edits()`. Browsing panel cannot close during edit. Remove Duplicates (the edit-bar button and the row menu entry under Remove from Playlist, both `EditorMessage::RemoveDuplicates`) filters the WHOLE buffer through `nokkvi_data::utils::dedupe::duplicate_entry_ids` (first copy of each song id stays; an active search, the clicked row and the multi-selection never narrow it) and is staged like every edit until Save.
- **Rules** — the smart-playlist rules editor (`views/playlist_editor/rules_view.rs`; handlers `update/rules_editor.rs` + `.nsp` import `update/nsp_import.rs`; root msgs `Message::RulesEditor(RulesEditorMessage)` + `NspImportPicked`). Two-pane `[results/preview | rules form]` layout with nested rule groups, chip cells, and an inline calendar date-picker overlay; no browsing panel. The preview pane owns its own columns dropdown (`OpenMenu::CheckboxDropdownPreview` — no `View` variant, parallel to Similar). Reached via `PlaylistContextEntry::EditRules` and the Playlists-header create menu (`OpenMenu::PlaylistsCreate`: New Playlist / New Smart Playlist / Import .nsp…). Editing-mode keys route through `rules_session_key_intercept` (see gotchas.md — bare-modifier keydowns). A free-text `Refresh` row (per-playlist refresh delay) sits under `Limit` and renders when `caps.per_playlist_refresh_delay` or the rules already carry a delay; preview-draft bodies go through `SmartRules::to_preview_value` (via `RulesSessionUi::draft_preview_body`), which strips the delay, while Save keeps `to_value`.
...
- **Single entry point**: F11 (`HotkeyAction::ToggleTheater`), the Queue cover's hover-revealed corner icon and its "Enter Theater Mode" menu row (`QueueAction::EnterTheater`), and the `theater` IPC verb all call `Nokkvi::enter_theater` / `toggle_theater`. Entering and leaving are unmount edges (see gotchas.md "Widget Tree & Focus").
- **Key policy**: `theater_key_policy(HotkeyAction)` is an exhaustive match: Passthrough (transport, modes, seek, lyrics, visualizer, the toggle, Escape), ExitThenPerform (view switches, `/`, Ctrl+E, Shift+C, Trawl, Roulette, refresh, save-queue, Find Similar / Top Songs, which act on the PLAYING track), ExitOnly (anything aimed at the hidden centred row or a hidden Settings row). `theater_key_policy_here` refines it: a toggle whose hidden target is already on (backtick over Settings, Ctrl+E with the split view open) only leaves. Wiring in `handle_raw_key_event`: the rules-editor intercept stands down while active; after the modal allowlist and only without a blocking modal, a Captured Escape is swallowed (it belongs to the overlay menu that captured it) and the policy applies; ExitThenPerform RE-RUNS the raw key after the exit, never `self.update(msg)`. `horizontal_arrow_owner()` returns `View` in theater, so bare arrows seek even over Settings. Escape's exit sits in `handle_clear_search` after the text-dialog tier. A held toggle key's repeats are dropped (`RawKeyEvent` carries winit's repeat flag).
- **Mouse-route safety net**: `handle_switch_view` exits first, and the root `update` exits whenever a message changes `theater_route_snapshot()` (view, browsing tab, editor presence). Find Similar / Top Songs exit explicitly (a re-run on an open Similar tab changes no route).
- **Widened gates**: `lyrics_surface_visible()` (Queue view OR theater) gates every lyrics resolve / next-track prefetch / backdrop blur; `theater_large_art_task` loads the playing album's large art on entry and on song change.
- **Layout**: `theater_view` is a FIXED four-layer `Stack` (panel, two `bottom_band_layers` placeholders-or-real, chrome), so the bar's slider/marquee state survives `v`. The chrome (corner exit icon + the unchanged player bar) is translated by `OverflowPin`, never resized. The chrome flips only in `theater::tick` (per frame, from the boat tick): `chrome_target` (Theater Controls; auto-hide = focused && (bar/corner hovered || menu open || activity within `HIDE_DELAY`)), eased by `slide_offset`. `cursor_hidden` is its own rule. Activity comes from a subscription present only while active, plus the key stamp.
- **Content**: `theater_now_playing_cover` (station art for radio; the playing album's art, frosted only while the lyrics layer shows). The lyric sheet sets `fit_to_panel`, so the text scales with the panel (`panel_fit_scale`, 1x..2.5x); the Queue cover keeps the fixed size. Cover Art hidden in theater paints `cover_backdrop()` and forces `PanelShape::Fill`.
- **Window**: Theater Fills the Screen queries the mode on entry; `PriorModeKnown` stores it and requests fullscreen only while still active on the same window; exit restores and consumes it. Close-to-tray (both paths) and quit-by-close use `exit_theater_for_closing_window` (drop, never restore); logout / session expiry exit via `reset_session_state`.
...
**`update/loader_target.rs`** — paged-loader unification:
...
- **Trawl (mix builder)**: whole-library seed search over the persistent `TrawlCrate` on `Nokkvi.trawl_crate` (`widgets/trawl_modal.rs`, `update/trawl_modal.rs`). State on `Nokkvi.trawl_modal`; opened from Harbour's anchor row or via context-menu "Add to Mix" accrual. Rows derive from ONE builder (`build_trawl_rows`) for render/activation parity; search stale-drop generation is root-owned (`trawl_search_generation`) so close/reopen can't re-mint captured generations. Enter toggles the centered seed; Ctrl+Enter maps to Play Mix in its slot-list intercept. Shift+Tab / Shift+Backspace step `TrawlModalState.tray_cursor` around a 6-position ring (None + the 5 tray controls) and the horizontal arrows — BOTH the bare seek pair and the Shift sort pair — cycle the focused control's value through its const `ALL` array via the existing `Set*` write path; `/` and typing clear the ring. INVARIANT: the trawl branch sits FIRST in `handle_settings_category_motion`, and in `Nokkvi::horizontal_arrow_context` (`update/hotkeys/navigation.rs`), which BOTH `handle_cycle_sort_mode` and `handle_seek_step` call as their first statement — above the SFX play and `reveal_current_toolbar()` — or the obscured background view gets stray SFX and a stranded toolbar reveal-lock; the gate arms live inside the trawl-gated `is_trawl_nav` allowlist only (`CycleSortMode(_)` and `SeekStep(_)` both).
...
- **Font / Theme pickers** (settings sub-lists): centered modal overlays over the settings panel — see settings-view.md for picker internals.
