...
| Harbour Sea | `harbour_sea.rs` | Procedural day/night seascape for the Harbour Trawl panel's artwork slot. `sea_bars(phase)` yields one coherent bar array per tick (advanced by the boat tick at `SEA_DRIFT_HZ`, no audio needed), fed to a canvas sea + the reused `boat_overlay` longship, with SVG sun/moon layers. Rendered via `trawl_scene(...)` from `views/harbour.rs`. Underwater life: drifting fish school (night starlight catch-rim), kelp beds with root seeps, rock mounds + starfish + a sunken cargo crate (the bed's landmark, fixed CRATE_* consts), a bubble stream rising from the trawled anchor (`BoatState::trawled_anchor_x()` = the SINGLE shared source for sprite placement AND the stream), night moonbeam shafts (banding-floor quiet, the 0.72–0.86 separator's only furniture), and a rare cycle-hashed serpent glide through the deep lane. Day parity: crest catch-light + shimmer gild in `logo_wood()` gold (ruled the SAME warm hue as sun/lantern, not a second note), sun glitter dashes, rare distant-sail cameo. All params const-seeded, integer-rate, test-pinned. History: a "treasure gems + crate data layer" concept shipped first and was retired on owner sight (read as fallen stars) |
