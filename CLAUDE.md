...
- **Artwork**: use `iced::widget::image::Handle::from_bytes(data)` for refreshable artwork — `Handle::from_path` keys on path and produces stale GPU textures when the file is overwritten. After every `put()` / `get()` on the artwork LRU, call `refresh_album_art_snapshot()` (for queue thumbs) or `refresh_large_artwork_snapshot()` (for the now-playing pane) so `ViewData` borrows the new map.
