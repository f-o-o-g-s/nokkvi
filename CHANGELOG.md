...
- Track-load HTTP probe (HEAD + first range request) no longer holds the engine lock — the position bar, MPRIS, mode toggles, and visualizer engine reads stay responsive during play actions on slow networks. Rapid skip-mashing now resolves to the last requested track instead of briefly playing intermediate ones.
- Visualizer FFT thread no longer stalls on the flash-effect path when the UI/render thread is mid-update — the per-frame flash update now skips a frame on lock contention instead of waiting, consistent with the rest of the FFT pipeline.
