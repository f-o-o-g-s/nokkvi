...
impl Nokkvi {
    ...
    /// gated on an actual change (PartialEq — validate() guarantees no NaN)
    /// so unrelated settings writes don't force an FFT-engine reinit.
    /// Shared by the full settings-loaded path and the slim
    /// `apply_visualizer_settings` path.
    pub(crate) fn push_visualizer_to_shared(
        &self,
        visualizer: &nokkvi_data::types::visualizer_config::VisualizerConfig,
    ) {
        use crate::visualizer_config::SharedVisualizerConfigExt;
        if self.visualizer_config.snapshot() != *visualizer {
            self.visualizer_config.apply(visualizer.clone());
    ...
    pub(crate) fn handle_player_settings_loaded(
        ...
        // Push the visualizer behavior config onto the shared render-path
        // state — gated on an actual change so unrelated settings writes
        // don't force an FFT-engine reinit. This is the unified replacement
        // for the legacy standalone VisualizerConfigChanged reload.
        self.push_visualizer_to_shared(&settings.visualizer);

        // Restore every view's column visibility from persisted settings.
        // Each `restore_from` is generated by `define_view_columns!` in lockstep
        // with the WRITE path (`ColumnPersist::apply_to_settings`), so the read
        // and write directions can never drift — adding a column emits both.
        self.queue_page.column_visibility =
            views::queue::QueueColumnVisibility::restore_from(&settings);
        self.artists_page.column_visibility =
            views::artists::ArtistsColumnVisibility::restore_from(&settings);
        self.genres_page.column_visibility =
            views::genres::GenresColumnVisibility::restore_from(&settings);
        self.playlists_page.column_visibility =
            views::playlists::PlaylistsColumnVisibility::restore_from(&settings);
        self.albums_page.column_visibility =
            views::albums::AlbumsColumnVisibility::restore_from(&settings);
        self.songs_page.column_visibility =
