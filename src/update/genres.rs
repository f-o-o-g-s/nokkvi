...
impl Nokkvi {
    ...
    pub(crate) fn handle_load_genre_collage(&mut self, genre_id: String) -> Task<Message> {
        if self.artwork.genre.collage.snapshot.contains_key(&genre_id)
    ...
    pub(crate) fn handle_genres(&mut self, msg: views::GenresMessage) -> Task<Message> {
        ...
        if let Some(loaded_id) = pin_after_albums {
            self.pin_after_load(
                ...
                |app, id| app.library.genres.iter().position(|g| g.id == id),
                |app, idx| {
                    let total = app.genres_page.expansion.flattened_len(&app.library.genres);
                    app.genres_page.common.slot_list.pin_selected(idx, total);
                },
            );
        }
        ...
        match action {
            ...
            GenresAction::PlayBatch(payload, force) => {
                self.genres_page
                    .common
                    .slot_list
